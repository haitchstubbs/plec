...
describe('navigation benchmark aggregation', () => {
  it('aggregates valid timings and preserves unavailable metric reasons', () => {
    const report = aggregatePhase([
      {
        metrics: { ttfb: 10, lcp: undefined },
        missing: { lcp: ['no LCP entry'] },
      },
      { metrics: { ttfb: 20, lcp: 30 }, missing: {} },
    ]);
    expect(report.metrics.ttfb).toMatchObject({
      median: 10,
      p95: 20,
      samples: 2,
    });
    expect(report.metrics.lcp).toMatchObject({
      median: 30,
      p95: 30,
      samples: 1,
    });
