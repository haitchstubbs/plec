...
const sampleCount = Number(process.env.PLEC_BENCH_SAMPLES ?? 10);
const port = 3199;
...
async function waitForLocalServer() {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    try {
      if ((await fetch(`${localBase}/`)).ok) return;
    } catch {}
    await sleep(100);
  }
...
async function sample(browser, phase) {
  ...
  try {
    ...
    const snapshot = await page.evaluate(() =>
      window.__plecPerformance?.snapshot(),
    );
    if (!snapshot)
      throw new Error('Plec performance API was not installed');
...
async function main() {
  ...
  try {
    ...
    for (const phase of configuredPhases(origin)) {
      const resolved =
        phase.id === 'deployed-origin'
          ? phase
          : { ...phase, url: `${localBase}/` };
      const samples = [];
      for (let index = 0; index < sampleCount; index += 1) {
        process.stdout.write(
          `[plec-bench] ${resolved.id} ${index + 1}/${sampleCount}\n`,
        );
