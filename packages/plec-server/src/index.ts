...
type Route = {
  id: string; path: string; graphId: string; outletId: string;
  parentId?: string; pendingGraphId?: string; errorGraphId?: string; loaderAction?: number;
  ...
  return createServer(async (incoming, outgoing) => {
    ...
    if (isDocumentRequest(context.pathname)) {
      try {
        ...
        const gate: SsrRenderGate = { development: options.development === true, gated: new Set() };
        const loader = match ? await executeRouteLoader(artifact, match.route, context) : undefined;
        const rendered = renderApplication(artifact, match?.route, context, gate, loader);
        const bootstrap = JSON.stringify(bootstrapPayload(artifact, match, context, loader, rendered.branches, rendered.loops, rendered.childGraph)).replace(/</g, '\\u003c');
        return sendHtml(outgoing, document, rendered.body, bootstrap, options, gate.development ? [...gate.gated] : []);
      } catch (error) {
...
function sendHtml(response: ServerResponse, metadata: DocumentMetadata, body: string, bootstrap: string, options: PlecServerOptions, gating: string[] = []) {
  ...
  response.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-cache', ...(gating.length ? { 'x-plec-ssr-gating': gating.join(',') } : {}) });
...
type LoaderOutcome = {
  graphId: string;
  action: number;
  state: { kind: 'resolved'; value: unknown } | { kind: 'rejected'; message: string };
};
...
async function executeRouteLoader(bundle: ArtifactBundle, route: Route, context: RequestContext): Promise<LoaderOutcome | undefined> {
  const action = route.loaderAction;
  if (action === undefined) return undefined;
  const graph = bundle.graphs.find((entry) => entry.graphId === route.graphId)?.graph;
  ...
  const fetchRequest = program?.routeLoader
    ? program.instructions.find((instruction) => instruction.op === 'capabilityRequest' && instruction.capability === 'fetch')?.request as
        | { url?: number; method?: string; decode?: string; requireOk?: boolean }
        | undefined
    : undefined;
  if (!component || !fetchRequest) throw new Error(`route loader action is invalid for ${route.id}`);
  const rawUrl = evaluate(component, typeof fetchRequest.url === 'number' ? fetchRequest.url : -1, { request: context, path: 'root', states: [] });
  const url = new URL(String(rawUrl), context.url);
  const rejected = (message: string): LoaderOutcome => ({ graphId: route.graphId, action, state: { kind: 'rejected', message } });
  try {
    const response = await fetch(url, { method: fetchRequest.method ?? 'GET' });
    if ((fetchRequest.requireOk ?? false) && !response.ok) {
      return rejected(`fetch ${url.pathname} failed with status ${response.status}`);
    }
    if (fetchRequest.decode !== 'responseJson') {
      throw new Error(`unsupported route loader decode ${String(fetchRequest.decode)}`);
    }
    return { graphId: route.graphId, action, state: { kind: 'resolved', value: await response.json() } };
  } catch (error) {
    return rejected(`fetch ${url.pathname} failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function bootstrapPayload(bundle: ArtifactBundle, match: ReturnType<typeof matchRoute> | undefined, context: RequestContext, loader: LoaderOutcome | undefined, branches: SsrBranchGraph, loops: SsrLoopGraph, childGraph: { instance: string; graphId: string } | undefined) {
  if (!match?.route) {
    return {
  ...
  const structure = (instance: string, graphId: string) => ({
    ...
    ...(loopRecords(instance).length ? { loops: loopRecords(instance) } : {}),
  });
  return {
    version: 2,
    snapshot: {
      ...
      routes: [{ routeId: match.route.id, params: match.params, phase: loader?.state.kind === 'rejected' ? 'error' : 'active' }],
      ...
      public: { location: `${context.pathname}${new URL(context.url).search}`, exports: {} },
      loaders: loader ? [loader] : [],
      structure: {
          graphs: {
            'root/outlet:main': structure('root/outlet:main', bundle.manifest.rootGraphId),
            ...(childGraph ? { [childGraph.instance]: structure(childGraph.instance, childGraph.graphId) } : {}),
          },
      },
    },
  };
...
function renderApplication(bundle: ArtifactBundle, route: Route | undefined, request: RequestContext, gate: SsrRenderGate, loader?: LoaderOutcome): { body: string; branches: SsrBranchGraph; loops: SsrLoopGraph; childGraph?: { instance: string; graphId: string } } {
  ...
  const routeGraphId = loader?.state.kind === 'rejected' && route?.errorGraphId ? route.errorGraphId : route?.graphId;
  const child = route && routeGraphId ? graph.get(routeGraphId) : undefined;
  ...
  // verbatim.
  const branches: SsrBranchGraph = new Map([['root/outlet:main', new Map()]]);
  const loops: SsrLoopGraph = new Map([['root/outlet:main', new Map()]]);
  const childGraph = child && route
    ? {
  ...
  const body = renderApp(root, {
    request, outlet: child, path: 'root', gate,
    loaderData: loader?.state.kind === 'resolved' ? loader.state.value : undefined,
    instance: 'root/outlet:main', rootComponent: root.rootComponent, branches, loops,
  });
  return { body, branches, loops, childGraph };
}
type Scope = { request: RequestContext; outlet?: App; path: string; props?: unknown[]; componentProps?: Record<number, { app: App; component: number }>; states?: unknown[]; row?: Record<string, unknown>; rowIndex?: number; rowKey?: string; rowRoot?: boolean; slot?: { app: App; component: number; nodes: number[]; scope: Scope }; gate?: SsrRenderGate; loaderData?: unknown; instance?: string; rootComponent?: number; branches?: SsrBranchGraph; loops?: SsrLoopGraph };
...
function renderNode(app: App, componentIndex: number, index: number, scope: Scope): string {
  ...
  if (node.op === 'conditional') {
    ...
    const selected: BranchSide = truthy ? 'consequent' : node.alternate == null ? 'none' : 'alternate';
    ...
    if (scope.branches && scope.instance && componentIndex === scope.rootComponent && !scope.row) scope.branches.get(scope.instance)!.set(index, selected);
    const inner = truthy ? renderNode(app, componentIndex, node.consequent!, scope) : node.alternate == null ? '' : renderNode(app, componentIndex, node.alternate, scope);
  ...
  if (node.op === 'loop') {
    ...
    if (!loop) throw new Error(`missing loop ${componentIndex}:${node.loop}`);
    const values = evaluate(component, loop.sourceExpression, scope);
    if (!Array.isArray(values)) throw new Error('LOOP_SOURCE_NOT_ARRAY');
    const keys: string[] = [];
    const rows = values.map((value, rowIndex) => {
      if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new Error('LOOP_ROW_NOT_OBJECT');
      const row = value as Record<string, unknown>;
      const key = canonicalKey(evaluate(component, loop.keyExpression, { ...scope, row, rowIndex }));
      if (keys.includes(key)) throw new Error(`DUPLICATE_LOOP_KEY:${key}`);
      keys.push(key);
    ...
    if (scope.loops && scope.instance && componentIndex === scope.rootComponent) scope.loops.get(scope.instance)!.set(index, keys);
    return rows;
  }
  ...
  if (scope.rowRoot && scope.rowKey !== undefined) attributes.push(`data-runtime-row-key="${escapeAttribute(scope.rowKey)}"`);
  attributes.push(`data-plec-node="${escapeAttribute(`${scope.path}/node:${index}`)}"`);
  const children = (node.children ?? []).map((child) => renderNode(app, componentIndex, child, { ...scope, rowRoot: false })).join('');
  const outlet = component.routeOutlets?.find((entry) => entry.node === index);
  const outletHtml = outlet && scope.outlet ? renderApp(scope.outlet, { ...scope, outlet: undefined, path: `${scope.path}/outlet:${outlet.id}`, instance: `${escapeInstanceSegment(scope.instance!)}/outlet:${escapeInstanceSegment(outlet.id)}`, rootComponent: scope.outlet.rootComponent }) : '';
...
function evaluate(component: Component, expression: number, scope: Scope): unknown {
  ...
  for (let pc = 0; pc < instructions.length; pc += 1) { const instruction = instructions[pc]!; switch (instruction.op) {
    ...
    case 'loadHost': { const slot = component.hostSlots?.[instruction.host as number]; const cookieName = slot?.name === undefined ? undefined : component.strings[slot.name]; if (slot?.kind === 'cookie') { if (scope.gate?.development) scope.gate.gated.add(`cookie:${cookieName ?? '<unnamed>'}`); stack.push(undefined); break; } if (slot?.kind === 'loaderData') { stack.push(scope.loaderData); break; } stack.push(slot?.kind === 'location' ? { pathname: scope.request.pathname, search: new URL(scope.request.url).search } : undefined); break; }
