...
function lowerJsxElement(node, parentId, state) {
    ...
    for (const attributeNode of expandIntrinsicAttributes(node.opening?.attributes ?? [], state)) {
        ...
        if (attributeNode.value.type === "StringLiteral") {
            ...
            propWrites.push({ name, staticValue: attributeNode.value.value, kind: "attribute" });
            continue;
        }
        if (attributeNode.value.type === "JSXExpressionContainer") {
            ...
            if (literalValue !== undefined && !state.preserveBindings) {
                ...
                propWrites.push({ name, staticValue, kind: isDomProperty(name) ? "property" : "attribute" });
            }
            else {
    ...
    if (propWrites.length)
        state.ir.propPrograms.push({ id: `p${state.ir.propPrograms.length + 1}`, targetId: elementId, writes: propWrites });
...
function lowerEvent(name, expression, targetId, state) {
    ...
    if (!call || !actionId) {
        ...
        if (!callbackName) {
            reportUnsupported(state, "UNSUPPORTED_EVENT", "Event handler must be a callback identifier or a declarative call.");
            return;
        }
        state.eventCounter += 1;
...
function reactEventName(name) {
    ...
    return { mouseenter: "mouseenter", mouseleave: "mouseleave", mousedown: "mousedown", mouseup: "mouseup", doubleclick: "dblclick", focus: "focus", blur: "blur" }[normalized] ?? normalized;
}
function isDomProperty(name) { return ["checked", "defaultChecked", "indeterminate", "value", "selected", "disabled", "readOnly"].includes(name); }
/**
    ...
    for (const original of getStatements(moduleAst)) {
        ...
        if (statement.type === "VariableDeclaration" || statement.type === "VarDecl")
            for (const declaration of statement.declarations ?? statement.decls ?? []) {
                ...
                if (name && isComponentName(name) && implementation)
                    state.components.set(componentKey(moduleId, name), { name, body: getFunctionBody(implementation), params: implementation.params ?? [], moduleId });
            }
    }
}
function unwrapComponentFactory(init) {
    if (init?.type === "ArrowFunctionExpression" || init?.type === "FunctionExpression")
        return init;
    const callee = getNodeName(init?.callee) ?? getNodeName(init?.callee?.property);
    if (callee === "forwardRef" && init.arguments?.[0])
        return unwrapExpression(init.arguments[0].expression ?? init.arguments[0]);
    return null;
}
function collectExports(moduleAst, state, moduleId) {
    const entries = state.exports.get(moduleId) ?? new Map();
    for (const statement of getStatements(moduleAst)) {
        if (statement.type === "ExportDeclaration") {
            ...
            if (name)
                entries.set(name, { moduleId, exportName: name });
        }
        if (statement.type !== "ExportNamedDeclaration" && statement.type !== "ExportNamedSpecifier" && statement.type !== "ExportAllDeclaration")
            continue;
...
function getLocalComponent(name, state) {
    ...
    if (imported) {
        ...
        if (resolved.exportName === "default")
            return [...state.components.values()].find((component) => component.moduleId === resolved.moduleId);
    }
...
function resolveExport(moduleId, name, state) {
    const [head, ...tail] = name.split(".");
    const found = state.exports.get(moduleId)?.get(head ?? name);
    if (!found)
        return { moduleId, exportName: name };
    if (!tail.length)
        return found;
...
function lowerToggleRoot(node, parentId, state) {
    ...
    const input = { id: inputId, tag: "input", parentId: rootId, attributes: [{ name: "type", staticValue: "checkbox" }], children: [] };
