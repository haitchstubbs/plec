...
function withMissingBindingsFilled(
  ...
  return {
    ...
    compileQuery:
      binding.compileQuery ?? browserFallbackBinding.compileQuery,
...
export function loadNativeBinding(): RuntimeBinding {
  ...
  if (explicitPath) {
    return withMissingBindingsFilled(
      require(explicitPath) as RuntimeBinding,
    );
  }
  ...
  for (const candidate of candidates) {
    try {
      return withMissingBindingsFilled(
        require(candidate) as RuntimeBinding,
      );
    } catch (err) {
