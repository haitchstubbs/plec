...
> Status snapshot as of August 2026. Reflects what is *implemented today*, not design targets.

## TL;DR

- SSR is **not a compiler feature**. The Rust compiler has zero SSR-specific code — it emits deterministic component graphs plus a route manifest, and SSR *emerges* from three components agreeing on one contract.
...
- Inert but notable: `ExecutionOwner { Shared, Server, Client }` + `PublicExport` (`crates/plec-ir/src/lib.rs:11`) model a server boundary in schema only — nothing wires it into the pipeline yet.
...
- `renderNode` (`index.ts:196`): recursive interpretation of the graph, including a small stack-VM `evaluate` (`index.ts:235`) for expressions. Host slots are request-aware — `location` resolves from the URL, `cookie` from request cookies, so `useLocation()` and `cookie.getSync` render real server values.
...
Slot children render under the *caller's* path — the caller's own adopt pass claims them directly, which is why slot-child mounting is skipped under adoption (`typed/runtime.rs:480`).
...
The claim walk itself (`TypedRuntime::adopt`, `typed/runtime.rs:841`) is deliberately paranoid, per its own doc comment: *"an absent or stale marker is an adoption failure, never an excuse to silently attach a listener to a guessed node."*
...
- Finishes by re-executing every binding against client-evaluated state and queueing listeners — the DOM is *overwritten*, not diffed, so server/client value drift is silently self-healed.
...
- Text claiming assumes the text node is *immediately* after its marker — any proxy/minifier/extension inserting whitespace between them breaks every downstream binding.
...
- Post-adoption `apply_static_bindings` overwrites server values with client re-evaluation — mismatch (time, randomness, cookie differences, rewritten URLs) is invisible; only tags and markers are checked.
...
- `phases.dedup()` only removes *adjacent* duplicates; the shipped artifact contains `FullstackLayout` twice (harmless today, latent confusion later).
...
| `stale-revision` / `route-mismatch` | Browser-glue gates (not runtime errors) | revision skew, path inequality |
...
| Runtime | `packages/plec-runtime/crates/runtime/src/runtime/lifecycle.rs` | `start_adopt` / `abandon_adoption` |
| Runtime | `packages/plec-runtime/crates/runtime/src/router/navigation.rs` | Route-chain adoption, outlet resolution |
| Runtime | `packages/plec-runtime/crates/runtime/src/typed/runtime.rs` | `TypedRuntime::adopt` claim walk |
