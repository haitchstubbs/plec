...
/// first stdout output". Both prefixes must stay byte-identical to the
/// lines `packages/plec-node-runtime/src/runtime.ts` writes (the READY
/// line carries the JSON payload after `": "`; the error line after the
/// single space).
